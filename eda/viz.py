...
def meanChart(total_df, sgg_nm):
    ...
    filtered_df = filtered_df[filtered_df['DEAL_YMD'].between(
        "20240501", "20240623")]
    result = filtered_df.groupby(['DEAL_YMD', 'HOUSE_TYPE'])[
        'OBJ_AMT'].agg('mean').reset_index()
    ...
    fig.update_yaxes(tickformat=".of",
                     title_text="물건가격(만원)",
...
def cntChart(total_df, sgg_nm):
    ...
    filtered_df = filtered_df[filtered_df['DEAL_YMD'].between(
        "20240501", "20240623")]
    result = filtered_df.groupby(['DEAL_YMD', 'HOUSE_TYPE'])[
        'OBJ_AMT'].count().reset_index().rename(columns={'OBJ_AMT': '거래건수'})
...
def barChart(total_df):
    ...
    month_selected = st.selectbox("월을 선택하세요.", [5, 6])
    house_selected = st.selectbox(
        "가구 유형을 선택하세요", total_df['HOUSE_TYPE'].unique())
    total_df['month'] = total_df['DEAL_YMD'].dt.month
    result = total_df[(total_df['month'] == month_selected)
                      & (total_df['HOUSE_TYPE'] == house_selected)]
    ...
    cnt_df = result.groupby(['SGG_NM', 'HOUSE_TYPE'])['OBJ_AMT'].count(
    ).reset_index().rename(columns={'OBJ_AMT': '거래건수'})
...
def showViz(total_df):
    ...
    selected = st.sidebar.radio(
        "차트 메뉴", ['가구당 평균 가격 추세', '가구당 거래 건수', '지역별 평균 가격 막대 그래프'])
