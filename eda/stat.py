...
def twoMeans(total_df):
    ...
    apt_df = total_df[(total_df['HOUSE_TYPE'] == '아파트') &
                      (total_df['month'].isin([5, 6]))]
    st.markdown("### 집계 \n"
                "- 5월과 6월의 아파트 가격을 비교한다.")
    ttest_df = round(apt_df.groupby('month')[
                     'OBJ_AMT'].agg(["mean", "std", "size"]), 1)
    st.dataframe(ttest_df, use_container_width=True)

    st.markdown("###  서울시 통합 5월 vs 6월 차이 검정\n"
                "- 5월과 6월의 아파트 평균 가격의 차이를 검정한다. \n"
                "- 가설설정 \n"
                "   + 귀무가설 : $H_{0}$: 5월과 6월의 아파트 평균 차이는 없다. \n"
                "   + 대립가설 : $H_{1}$: 5월과 6월의 아파트 평균 차이는 있다. \n")

    march_df = apt_df[apt_df['month'] == 5]
    april_df = apt_df[apt_df['month'] == 6]
    result = ttest(march_df['OBJ_AMT'], april_df['OBJ_AMT'], paired=False)
    st.dataframe(result, use_container_width=True)
    st.markdown(
        f"- 확인결과 p-value 값이 {result['p-val'].values[0]} 이므로 $H_{0}$을 채택하여, 5월과 6월의 아파트 평균 차이는 없다.")

    selected_sgg_nm = st.sidebar.selectbox(
        "자치구명", sorted(total_df['SGG_NM'].unique()))
    st.markdown(f"### 서울시 {selected_sgg_nm} 5월 vs 6월 차이 검정\n"
                "- 자치구를 선택하여 5월과 6월의 아파트 평균 차이가 있는지 확인하도록 한다.")

    sgg_df = apt_df[apt_df['SGG_NM'] == selected_sgg_nm]
    sgg_march_df = sgg_df[sgg_df['month'] == 5]
    sgg_april_df = sgg_df[sgg_df['month'] == 6]
    sgg_result = ttest(sgg_march_df['OBJ_AMT'],
                       sgg_april_df['OBJ_AMT'], paired=False)
    st.dataframe(sgg_result, use_container_width=True)
    if sgg_result['p-val'].values[0] > 0.05:
        st.markdown(
            f"- 확인결과 p-value 값이 {sgg_result['p-val'].values[0]} 이므로 $H_{0}$을 채택하여, 5월과 6월의 아파트 평균 차이는 없다.")
    else:
        st.markdown(
            f"- 확인결과 p-value 값이 {sgg_result['p-val'].values[0]} 이므로 $H_{1}$을 채택하여, 5월과 6월의 아파트 평균 차이는 있다.")

    st.markdown(
        f"### 서울시 :blue[{selected_sgg_nm}] 5월 vs 6월 시각화", unsafe_allow_html=True)
    fig, ax = plt.subplots(figsize=(10, 3))
    sns.pointplot(x='month', y='OBJ_AMT', data=sgg_df)
    sns.despine()
    st.pyplot(fig)
    st.dataframe(round(sgg_df.groupby('month')['OBJ_AMT'].agg(
        ["mean", "std", "size"]), 1), use_container_width=True)
...
def corrRelation(total_df):
    ...
    st.markdown("### 상관관계 분석을 위한 데이터 확인 \n"
                ...
                "- 먼저 추출된 데이터를 확인한다.")
    corr_df = apt_df[['DEAL_YMD', 'OBJ_AMT', 'BLDG_AREA',
                      'SGG_NM', 'month']].reset_index(drop=True)
    ...
    st.markdown("### 상관관계 계수 및 검정 \n"
                "- 계수를 확인한다. \n")
    st.dataframe(pg.corr(corr_df['BLDG_AREA'], corr_df['OBJ_AMT']).round(
        3), use_container_width=True)
    st.markdown("- 상관계수는 0.573 이며 건물면적이 증가할 때 마다, 물건금액도 같이 증가하는 경향성을 나타나는 것을 확인하라 수 있다. \n"
                "그렇다면, 각 자치구별로 상관관계 시각화 및 상관계수는 어떻게 다른지 확인해본다. \n")
    selected_sgg_nm = st.sidebar.selectbox(
        "자치구명", sorted(corr_df['SGG_NM'].unique()))
    selected_month = st.sidebar.selectbox(
        "월", sorted(corr_df['month'].unique()))
    st.markdown(f"### 서울시 {selected_sgg_nm} {selected_month}월 아파트 가격 ~ 건물면적 상관관계 분석\n"
                "- 각 자치구 및 월별 시각화 및 상관계수를 표시할 수 있다.")
    sgg_df = corr_df[(corr_df['SGG_NM'] == selected_sgg_nm)
                     & (corr_df['month'] == selected_month)]
...
def regRession(total_df):
    ...
    selected_month = st.sidebar.selectbox(
        "월", sorted(corr_df['month'].unique()))
    reg_df = corr_df[(corr_df['SGG_NM'] == selected_sgg_nm)
                     & (corr_df['month'] == selected_month)]
    st.markdown("### 데이터 확인")
...
def showStat(total_df):
    total_df['DEAL_YMD'] = pd.to_datetime(
        total_df['DEAL_YMD'], format="%Y%m%d")
