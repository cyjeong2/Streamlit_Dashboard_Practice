...
def mapMatplotlib(merge_df):
    ...
    merge_df[merge_df['month'] == 5].plot(ax=ax[0], column="mean", cmap="Pastel1", legend=False, alpha=0.9,
                                          edgecolor='gray')
    merge_df[merge_df['month'] == 6].plot(ax=ax[1], column="mean", cmap="Pastel1", legend=False, alpha=0.9,
                                          edgecolor='gray')
    ...
    ax[0].set_title('2024-5월 아파트 평균(만원)')
    ax[1].set_title('2024-6월 아파트 평균(만원)')
    ax[0].set_axis_off()
    ax[1].set_axis_off()
...
def mapPlotly(merge_df):
    ...
    st.sidebar.caption(
        "Site : https://plotly.com/python/mapbox-layers/#mapbox-basemap-style-references")
    ...
    fig.update_traces(
        hovertemplate='<b>%{location}</b><br>아파트 평균 가격: %{z:,.0f}(만원)')
...
def showMap(total_df):
    ...
    total_df = total_df[['DEAL_YMD', 'month', 'SGG_CD',
                         'SGG_NM', 'OBJ_AMT', 'HOUSE_TYPE']].reset_index(drop=True)

    summary_df = total_df.groupby(['SGG_CD', 'month'])['OBJ_AMT'].agg([
