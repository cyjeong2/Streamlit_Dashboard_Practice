...
def predict_plot(total_df, types, periods):
    ...
    for i in range(0, len(types)):
        model = Prophet()
        total_df2 = total_df.loc[total_df['HOUSE_TYPE']
                                 == types[i], ["DEAL_YMD", "OBJ_AMT"]]
        result_df = total_df2.groupby(
            'DEAL_YMD')['OBJ_AMT'].agg("mean").reset_index()
        result_df = result_df.rename(
            columns={"DEAL_YMD": "ds", "OBJ_AMT": "y"})
        model.fit(result_df)
