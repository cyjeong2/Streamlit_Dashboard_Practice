...
def run_home():
    ...
    selected_month = st.sidebar.radio("확인하고 싶은 월을 선택하세요 👇", ['5월', '6월'])
    month_dict = {'5월': 5, '6월': 6}
    ...
    sorted_df = filtered_month[[
        "SGG_NM", "BJDONG_NM", "BLDG_NM", "BLDG_AREA", "OBJ_AMT"]]
    st.dataframe(sorted_df.sort_values(
        by="OBJ_AMT", ascending=False).head(3).reset_index(drop=True))
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("### 아파트 가격 하위 3")
    sorted_df = filtered_month[[
        "SGG_NM", "BJDONG_NM", "BLDG_NM", "BLDG_AREA", "OBJ_AMT"]]
    st.dataframe(sorted_df.sort_values(
        by='OBJ_AMT', ascending=True).head(3).reset_index(drop=True))

    st.markdown("<hr>", unsafe_allow_html=True)
    st.caption(
        "출처 : [서울시 부동산 실거래가 정보](https://data.seoul.go.kr/dataList/0A-21275/S/1/datasetView.do)")
